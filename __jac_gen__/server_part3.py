...
class Router(_Jac.Node):
    ...
    def classify(self, message: str) -> ChatType:
        return _Jac.with_llm(file_loc=__file__, model=llm, model_params={'method': 'Reason', 'temperature': 0.0}, scope='server_part3(Module).Router(node).classify(Ability)', incl_info=[], excl_info=[], inputs=[('query from the user to be routed.', str, 'message', message)], outputs=('', 'ChatType'), action='route the query to the appropriate task type', _globals=globals(), _locals=locals())
...
class infer(_Jac.Walker):
    ...
    def init_router(self, _jac_here_: _Jac.RootType) -> None:
        ...
        _Jac.connect(left=router_node, right=HelpChat(), edge_spec=_Jac.build_edge(is_undirected=False, conn_type=None, conn_assign=None))
...
@_Jac.make_node(on_entry=[_Jac.DSFunc('respond', infer)], on_exit=[])
@__jac_dataclass__(eq=False)
class HelpChat(Chat, _Jac.Node):
    chat_type: ChatType = _Jac.has_instance_default(gen_func=lambda: ChatType.SUPPORT)

    def respond(self, _jac_here_: infer) -> None:

        def respond_with_llm(message: str, chat_history: list[dict], agent_role: str) -> str:
            return _Jac.with_llm(file_loc=__file__, model=llm, model_params={}, scope='server_part3(Module).HelpChat(node).respond(Ability).respond_with_llm(Ability)', incl_info=[], excl_info=[], inputs=[('current message', str, 'message', message), ('chat history', list[dict], 'chat_history', chat_history), ('role of the agent responding', str, 'agent_role', agent_role)], outputs=('response', 'str'), action='Respond to message using chat_history as context and agent_role as the goal of the agent', _globals=globals(), _locals=locals())
        _jac_here_.response = respond_with_llm(_jac_here_.message, _jac_here_.chat_history, agent_role='You are a conversation agent design to help users troubleshoot or get customer service.')